...
default sets \Rcode{width=90}, which is optimal for the default font size 11pt; for smaller/larger font size please consider increasing/decreasing this value .
...
\item \verb+\CRANpkg{data.table}+ for \R{} packages available on CRAN,
...
\item \verb+\Rclass{GRanges}+ when refering to a formal class
  \Rclass{GRanges}.
\item \verb+\Rcode{log(x)}+ for \R{} code.
\end{itemize}
%% 
Communication:
\begin{itemize}
...
General:
\begin{itemize}
\item \verb+\email{user@domain.com}+ to provide a linked email address, \email{user@domain.com}.
\item \verb+\file{script.R}+ for file names and file paths, \file{script.R}.
\end{itemize}
