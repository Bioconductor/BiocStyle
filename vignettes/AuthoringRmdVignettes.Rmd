...
In order to build BiocStyle .Rmd documents the R package `r CRANpkg("bookdown")` is necessary, which relies on `r CRANpkg("rmarkdown")` and [pandoc](http://johnmacfarlane.net/pandoc/) to compile the final output document.

All the required additional package dependencies can be installed by running

```{r rmarkdown, eval=FALSE}
source("https://bioconductor.org/biocLite.R")
biocLite("BiocStyle", dependencies = TRUE)
```

Unless you are using RStudio, you will also need a recent version of [pandoc](http://johnmacfarlane.net/pandoc/) (>= 1.12.3). See the [pandoc installation instructions](https://github.com/rstudio/rmarkdown/blob/master/PANDOC.md) for details on installing pandoc for your platform.


# Getting started

To enable the _Bioconductor_ style in your R Markdown vignettes you need to:
...
it is also possible to compile Bioconductor documents with the older markdown v1 engine 
...
`r CRANpkg('markdown')` differs from how this is done with `r CRANpkg('rmarkdown')`.
...
Author affiliations and emails can be specified in named sublists of the author list. Multiple affiliations per author can be specified this way.
...
A list of unique affiliations will be displayed in below the authors, similar as in this document.
...
and later referenced with an asterisk *. For example, the above specification is equivalent to the shorthand notation
...
These can be conveniently called inline, e.g., `r inline('Biocpkg("IRanges")')`.
...
Figures which have no captions are just placed wherever they were generated in the R code.
If you assign a figure caption to a code chunk via the chunk option `fig.cap`, the plot will be automatically labeled and numbered^[additionally, for PDF output they will be placed in figure environments], and can be also cross-referenced. These features are provided by `r CRANpkg("bookdown")`, which defines a cross-format syntax for specifying references. The figure label is generated from the code chunk label by prefixing it with `fig:`, e.g., the label of a figure originating from the chunk `foo` will be `fig:foo`. To reference a figure, use the syntax `\@ref(label)`^[do not forget the leading backslash!], where `label` is the figure label, e.g., `fig:foo`. For example, the following code chunk was used to produce Figure \@ref(fig:plot).
...
In addition to regular figures, *BiocStyle* provides small and wide figures specified by `fig.small` and `fig.wide` code chunk options.
...
Like figures, tables with captions will also be numbered and can be referenced. The caption can be entered as a paragraph starting with `Table:`^[or just `:`], which may appear either before or after the table. When adding labels, make sure that the label appears in the beginning of the table caption in the form `(\#tab:label)`, and use `\@ref(tab:label)` to refer to it. For example, Table \@ref(tab:table) has been produced with the following code.
...
To number and refer to equations, put them in equation environments and assign labels to them using the syntax `(\#eq:label)`^[due to technical constraints equation labels must start with `eq:`], e.g.,
...
Apart from referencing figures (Section \@ref(figures)), tables (Section \@ref(tables)), and equations (Section \@ref(equations)), you can also use the same syntax `\@ref(label)` to reference sections, where `label` is the section ID. By default, Pandoc will generate IDs for all section headers, e.g., `# Hello World` will have an ID `hello-world`. You can also manually assign an ID to a section header to make sure you do not forget to update the reference label after you change the section header, by simply appending `{#id}` to the section header.
