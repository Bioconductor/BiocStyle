...
\begin{verbatim}
    ...
    Suggests: BiocStyle
\end{verbatim}
the following to the header of the \texttt{.Rnw} file:
\begin{verbatim}
...
\end{verbatim}
\Biocpkg{BiocStyle} automatically attaches the following \LaTeX{} packages: \texttt{color}, \texttt{enumitem}, \texttt{fancyhdr}, \texttt{geometry}, \texttt{helvet}, \texttt{hyperref}, \texttt{natbib}, \texttt{parskip}, \texttt{sectsty}, and \texttt{titling}.
