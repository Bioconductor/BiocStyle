...
\begin{verbatim}
    Suggests: BiocStyle
\end{verbatim}
and add this code chunk to the header of your \texttt{.Rnw} file:
\begin{verbatim}
    <<style, eval=TRUE, echo=FALSE, results=tex>>=
    BiocStyle::latex()
    @ 
...
\begin{verbatim}
    ...
    Suggests: BiocStyle
\end{verbatim}
and the following to the header of the \texttt{.Rnw} file:
\begin{verbatim}
...
\end{verbatim}
\Biocpkg{BiocStyle} automatically attaches the following \LaTeX{} packages: \texttt{color}, \texttt{enumitem}, \texttt{fancyhdr}, \texttt{geometry}, \texttt{helvet}, \texttt{hyperref}, \texttt{parskip}, \texttt{sectsty}, and \texttt{titling}.
...
Communication:
\begin{itemize}
  \exitem{bioccomment}{comment to the user}{communicates a}
...
\begin{itemize}
  ...
  \exitem{cite}{Gentleman:2004}{for \software{unsrturl.bst} citations}
  This style supports hyperlinks to DOI and PubMed IDs but not
  \verb+\citet+ or \verb+\citep+.
\end{itemize}
