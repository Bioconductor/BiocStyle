...
\texttt{Bioconductor.sty} defines the following commands.
%% 
Software:
\begin{itemize}
...
%---------------------------------------------------------
 Besides the usual \LaTeX capabilities (\verb+figure+ environments for floats,
the \verb+includegraphics+ command)
\texttt{Bioconductor.sty} defines \verb+\myincfig{filename}{width}{shorttitle}{extendedcaption}+, which expects four arguments:

\begin{tabular}{lp{13cm}}
\texttt{filename}&the name of the figure file, which will also be used as the label by
    which the float can be refered to by \verb+\ref{}+\\
\texttt{width}&figure width\\
\texttt{shorttitle}&a short description of the figure, which will be used for the list of
     figures, and will be printed in bold as the first part of the figure caption\\
\texttt{extendedcaption}&continuation of the figure caption
\end{tabular}

\parindent0mm This is perhaps best demonstrated on an example, see Figure~\ref{LatexStyle-figureexample}.
<<figureexample,fig=TRUE,include=FALSE,width=4.2,height=4.6>>=
v = seq(0, 60i, length=1000)
plot(abs(v)*exp(v), type="l", col="Royalblue")
@ 

\myincfig{LatexStyle-figureexample}{0.25\textwidth}{Graph of a function.}{The code for this function
  is presented in the code chunk.}
\parindent0mm\verb+\myincfig{LatexStyle-figureexample}{0.25\textwidth}{Graph of a function.}%+\\
\verb+  {The code that creates this figure is shown in the code chunk.}+

%---------------------------------------------------------
%\section{Code listings}
%---------------------------------------------------------
%Code listing ~\ref{aCodeListing} is produced by the following \LaTeX command:
%
%\verb+\lstinputlisting[language=R,label=aCodeListing,breaklines=true,caption={Code from this package}]{../R/latex.R}+
%
%\lstinputlisting[language=R,label=aCodeListing,breaklines=true,caption={Code from this package}]{../R/latex.R}

%---------------------------------------------------------
\section{Session info}
%---------------------------------------------------------
 Below, the output is shown of \Rfunction{sessionInfo} on the system on which this document was compiled.
