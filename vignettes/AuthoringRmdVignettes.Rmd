...
_Bioconductor_ R Markdown format is build on top of _R_ package `r CRANpkg("bookdown")`, which in turn relies on `r CRANpkg("rmarkdown")` and [pandoc](http://johnmacfarlane.net/pandoc/) to compile the final output document.
Therefore, unless you are using RStudio, you will need a recent version of [pandoc](http://johnmacfarlane.net/pandoc/) (>= 1.12.3). See the [pandoc installation instructions](https://github.com/rstudio/rmarkdown/blob/master/PANDOC.md) for details on installing pandoc for your platform.
...
BiocStyle's `html_document2` and `pdf_document2` format functions extend the corresponding original _rmarkdown_ formats, so they accept the same arguments as `html_document` and `pdf_document`, respectively. For example, use `toc_float: true` to obtain a floating TOC as in this vignette.
...
To number and reference equations, put them in equation environments and assign labels to them following the syntax `(\#eq:label)`^[due to technical constraints equation labels must start with `eq:`], e.g.,
