...
Apart from the default markdown engine implemented in the `r
CRANpkg('rmarkdown')` package, it is also possible to compile _Bioconductor_
documents with the older markdown v1 engine from the package `r
CRANpkg('markdown')`. There are some differences in setup and the resulting
...
The `author` field allows for specifing author names along with affiliation and
email information.
...
possible to reference it. These features are provided by `r
CRANpkg("bookdown")`, which defines a format-independent syntax for specifying
...
image to visual impaired readers.  If you provide a figure caption with
...
was use to include the first image shown in this section, so that its intention
...
You may also want to consider the colour palette used in your figures, to ensure
they are accessible to colour impaired readers.  *BiocStyle* doesn't provide a
specific colour palette, but many options are available in packages from CRAN.
There have also been
[https://developer.r-project.org/Blog/public/2019/11/21/a-new-palette-for-r/](significant
efforts) made to improve the default colours provided by `palette()` in R-4.0.0.
Bioconductor contains some domain specific packages that focus on colour blind
friendly visualisations e.g. `r Biocpkg("dittoSeq")` for single-cell and bulk
RNA-sequencing data.
